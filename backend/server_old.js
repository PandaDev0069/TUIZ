...
// PLACEHOLDER - API routes replacement in progress
...
io.on('connection', (socket) => {
  ...
  // Submit answer (enhanced for better tracking)
  socket.on('submit_answer', ({ room, name, answer }) => {
    try {
      ...
      // Store the answer
      if (!activeGame.currentAnswers) {
        activeGame.currentAnswers = [];
      }
      activeGame.currentAnswers.push({
        playerId: socket.id,
        playerName: name,
        answer: answer,
        isCorrect: isCorrect,
      ...
      socket.to(activeGame.hostSocket).emit('player_answered', { name });
      ...
      console.log(`📝 ${name} answered question ${activeGame.currentQuestionIndex}: ${isCorrect ? 'correct' : 'incorrect'}`);
    } catch (error) {

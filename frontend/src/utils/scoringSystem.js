...
console.log('=== TUIZ Advanced Scoring System Examples ===');

// Example game settings configurations
const gameSettingsFixed = {
  pointCalculation: 'fixed',
  streakBonus: false,
  basePoints: 100
};

const gameSettingsTimeBonus = {
  pointCalculation: 'time-bonus',
  streakBonus: true,
  basePoints: 100
};

const gameSettingsStreakOnly = {
  pointCalculation: 'fixed',
  streakBonus: true,
  basePoints: 100
};
...
const test1 = calculateScore({
  ...
  timeTaken: 2,
  timeLimit: 10,
  gameSettings: gameSettingsFixed
});
console.log('Test 1 (fixed points, no bonuses):', test1);

// Test case 2: Time bonus + streak bonus enabled
const test2 = calculateScore({
  basePoints: 100,
  streakCount: 3,
  timeTaken: 2,
  timeLimit: 10,
  gameSettings: gameSettingsTimeBonus
});
console.log('Test 2 (time + streak bonus):', test2);

// Test case 3: Only streak bonus
const test3 = calculateScore({
  basePoints: 100,
  streakCount: 10,
  timeTaken: 9,
  timeLimit: 10,
  gameSettings: gameSettingsStreakOnly
});
console.log('Test 3 (streak bonus only):', test3);

// Test case 4: Backend-compatible function
...
const backendTest = calculateGameScore({
...
console.log('Backend integration test:', backendTest);

// Test case 5: Detailed breakdown
...
console.log('Detailed breakdown:', JSON.stringify(breakdown, null, 2));

// Test case 6: Maximum streak demonstration
const maxStreakTest = calculateScore({
  basePoints: 100,
  streakCount: 20, // Very high streak to show cap at 60%
  timeTaken: 1,
  timeLimit: 10,
  gameSettings: gameSettingsTimeBonus
});
console.log('Max streak test (20 streak):', maxStreakTest);

// Test case 7: Different time buckets
console.log('\n=== Time Bonus Testing ===');
[2, 5, 8, 10].forEach(time => {
  ...
  console.log(`Time taken: ${time}s/${10}s = ${score} points`);
});
